...
void Callback(const sensor_msgs::Imu::ConstPtr& data){
	if(_prevT != -1){
		if((data->header.stamp.nsecs - _prevT) >= 1000.0/_sampleRate){
			...
			MadgwickAHRSupdateIMU(qqq, gyro, acc);
			msg.orientation.w = qqq[0];
			msg.orientation.x = qqq[1];
			msg.orientation.y = qqq[2];
...
void MadgwickAHRSupdate(float* qtn, float* gyro, float* acc, float* mag){
	float q0=qtn[0], q1=qtn[1], q2=qtn[2], q3=qtn[3], gx=gyro[0], gy=gyro[1], gz=gyro[2], ax=acc[0], ay=acc[1], az=acc[2], mx=mag[0], my=mag[1], mz=mag[2];
	...
	if((mx == 0.0f) && (my == 0.0f) && (mz == 0.0f)) {
		MadgwickAHRSupdateIMU(qtn, gyro, acc);
		return;
	}
...
void MadgwickAHRSupdateIMU(float* qtn, float* gyro, float* acc){
	float q0=qtn[0], q1=qtn[1], q2=qtn[2], q3=qtn[3], gx=gyro[0], gy=gyro[1], gz=gyro[2], ax=acc[0], ay=acc[1], az=acc[2];
